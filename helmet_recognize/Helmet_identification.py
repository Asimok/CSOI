...
from helmet_recognize.html_detect_functions import single_image_test
...
def upload():
    if request.method == 'POST':
        ...
        if filetype == ".jpg" or filetype == ".png":
            upload_path = os.path.join(basepath, 'static/uploads/beforeimg',
                                       secure_filename(f.filename))  # 注意：没有的文件夹一定要先创建，不然会提示没有该路径
            f.save(upload_path)
            return redirect('/imgcontrast/' + secure_filename(f.filename))
        elif filetype == ".mp4" or filetype == ".avi":
            upload_path = os.path.join(basepath, 'static/uploads/beforevideo',
                                       secure_filename(f.filename))  # 注意：没有的文件夹一定要先创建，不然会提示没有该路径
            f.save(upload_path)
            print('video')
